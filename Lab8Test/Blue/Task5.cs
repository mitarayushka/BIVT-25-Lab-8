...
namespace Lab8Test.Blue
{
    ...
    public sealed class Task5
    {
        ...
        public void Test_00_OOP()
        {
            ...
            Assert.IsFalse(type.GetProperty("Surname")?.CanWrite ?? false,
                "Свойство Surname должно быть только для чтения");
            Assert.IsFalse(type.GetProperty("Place")?.CanWrite ?? false,
                "Свойство Place должно быть только для чтения");
            Assert.IsNotNull(
                type.GetConstructor(BindingFlags.Instance | BindingFlags.Public, null,
                    new[] { typeof(string), typeof(string) }, null),
                "Нет публичного конструктора Sportsman(string name, string surname)");
            Assert.IsNotNull(
                type.GetMethod("SetPlace", BindingFlags.Instance | BindingFlags.Public, null, new[] { typeof(int) },
                    null), "Нет публичного метода SetPlace(int place)");
            Assert.IsNotNull(
                type.GetMethod("Print", BindingFlags.Instance | BindingFlags.Public, null, Type.EmptyTypes, null),
            ...
            Assert.IsFalse(type.GetProperty("Name")?.CanWrite ?? false, "Свойство Name должно быть только для чтения");
            Assert.IsFalse(type.GetProperty("Sportsmen")?.CanWrite ?? false,
                "Свойство Sportsmen должно быть только для чтения");
            Assert.IsFalse(type.GetProperty("SummaryScore")?.CanWrite ?? false,
                "Свойство SummaryScore должно быть только для чтения");
            Assert.IsFalse(type.GetProperty("TopPlace")?.CanWrite ?? false,
                "Свойство TopPlace должно быть только для чтения");
            Assert.IsNotNull(
                type.GetMethod("Add", BindingFlags.Instance | BindingFlags.Public, null,
                    new[] { typeof(Lab8.Blue.Task5.Sportsman) }, null),
                "Нет публичного метода Add(Sportsman sportsman)");
            Assert.IsNotNull(
                type.GetMethod("Add", BindingFlags.Instance | BindingFlags.Public, null,
                    new[] { typeof(Lab8.Blue.Task5.Sportsman[]) }, null),
                "Нет публичного метода Add(Sportsman[] sportsmen)");
            Assert.IsNotNull(
                type.GetMethod("Sort", BindingFlags.Static | BindingFlags.Public, null,
                    new[] { typeof(Lab8.Blue.Task5.Team[]) }, null),
                "Нет публичного статического метода Sort(Team[] array)");
            Assert.IsNotNull(
                type.GetMethod("GetChampion", BindingFlags.Static | BindingFlags.Public, null,
                    new[] { typeof(Lab8.Blue.Task5.Team[]) }, null),
                "Нет публичного статического метода GetChampion(Team[] teams)");
            Assert.IsNotNull(
                type.GetMethod("Print", BindingFlags.Instance | BindingFlags.Public, null, Type.EmptyTypes, null),
                "Нет публичного метода Print()");
            var strengthMethod = type.GetMethod("GetTeamStrength",
                BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
            Assert.IsNotNull(strengthMethod, "Нет метода GetTeamStrength()");
            Assert.IsTrue(strengthMethod.IsFamily || strengthMethod.IsFamilyOrAssembly,
                "Метод GetTeamStrength должен быть protected");
        ...
        public void Test_08_GetTeamStrength()
        {
            ...
            // ManTeam 0
            var strength0 = (double)_manTeams[0].GetType()
                .GetMethod("GetTeamStrength", BindingFlags.Instance | BindingFlags.NonPublic)
                .Invoke(_manTeams[0], null);
            Assert.AreEqual(11.32075, strength0, delta);

            // WomanTeam 0
            var strengthWoman = (double)_womanTeams[0].GetType()
                .GetMethod("GetTeamStrength", BindingFlags.Instance | BindingFlags.NonPublic)
                .Invoke(_womanTeams[0], null);
            Assert.AreEqual(1.160714, strengthWoman, delta);

            // ManTeam 1
            var strength1 = (double)_manTeams[1].GetType()
                .GetMethod("GetTeamStrength", BindingFlags.Instance | BindingFlags.NonPublic)
                .Invoke(_manTeams[1], null);
        ...
        {
            _womanTeams = _inputWomanTeams.Select(t => (Lab8.Blue.Task5.Team)new Lab8.Blue.Task5.WomanTeam(t.Name))
                .ToArray();
        }
        ...
        private void CheckManTeams(bool filled)
        {
            for (int i = 0; i < _manTeams.Length; i++)
            {
                ...
                if (filled)
                {
                    ...
                    int expTop = team.Sportsmen.Where(s => s.Place > 0).Select(s => s.Place)
